...
class ChatAdapter(val listener: (ChatItem) -> Unit) : RecyclerView.Adapter<ChatAdapter.ChatItemViewHolder>() {
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ChatItemViewHolder {
        ...
        return when (viewType) {
            ...
            else -> ArhiveViewHolder(inflater.inflate(R.layout.item_chat_archive, parent, false))
    ...
    fun updateData(data: List<ChatItem>) {
        Log.d(
            "M_ChatAdapter",
            "update data adapter - new data ${data.size} hash: ${data.hashCode()}" + "old data:${items.size} hash: ${items.hashCode()}"
        )
        val diffCallback = object : DiffUtil.Callback() {
            override fun areItemsTheSame(oldPos: Int, newPos: Int): Boolean = items[oldPos].id == data[newPos].id

            override fun areContentsTheSame(oldPos: Int, newPos: Int): Boolean =
                items[oldPos].hashCode() == data[newPos].id.hashCode()
    ...
    inner class SingleViewHolder(convertView: View) : ChatItemViewHolder(convertView), ItemTouchViewHolder {
        ...
        override fun bind(item: ChatItem, listener: (ChatItem) -> Unit) {
                if (item.avatar == null) {
                    ...
                    iv_avatar_single.createAvatar(item.initials, Utils.convertDpToPixels(24), itemView.context.theme)
                } else {
                    Glide.with(itemView)
                        .load(item.avatar)
    ...
    inner class GroupViewHolder(convertView: View) : ChatItemViewHolder(convertView), ItemTouchViewHolder {
        ...
        override fun bind(item: ChatItem, listener: (ChatItem) -> Unit) {
            //TODO замена с мастерклассом
            iv_avatar_group.createAvatar(item.title[0].toString(), Utils.convertDpToPixels(32), itemView.context.theme)
